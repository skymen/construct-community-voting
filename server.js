...
const config = {
    ...
    redirectUri: process.env.REDIRECT_URI || `http://localhost:${PORT}/auth/discord/callback`,
...
function hasVotedThisMonth(userId) {
    ...
    return votes.votes.some(v => v.userId === userId && v.month === currentMonth);
}
...
function recordVote(userId, username, projectSlug, projectName) {
    ...
    if (votes.votes.some(v => v.userId === userId && v.month === currentMonth)) {
...
function requireRole(req, res, next) {
    if (!req.session.user?.hasRequiredRole) {
        return res.status(403).json({ error: 'You do not have the required role to vote' });
    }
    next();
}
...
app.post('/api/vote', requireAuth, requireRole, (req, res) => {
    ...
    if (!projectSlug || !projectName) {
        return res.status(400).json({ error: 'Project slug and name are required' });
    }
...
app.listen(PORT, () => {
    ...
    console.log('\n📋 Configuration:');
    console.log(`   - Discord Client ID: ${config.clientId ? '✓ Set' : '✗ Missing'}`);
    console.log(`   - Discord Client Secret: ${config.clientSecret ? '✓ Set' : '✗ Missing'}`);
    console.log(`   - Discord Bot Token: ${config.botToken ? '✓ Set' : '✗ Missing'}`);
